"""
Device Management Agent — WebSocket Lambda Handler
...
connections_table = dynamodb.Table(os.environ.get("CONNECTIONS_TABLE", "connected-care-dm-connections"))
...
def handler(event, context):
    ...
    if event.get("isOrchestrator"):
        ...
        if not prompt:
            return {"statusCode": 200, "body": json.dumps({"response": "Device Management Agent ready."})}
        result = agent(prompt)
    ...
    try:
        ...
        if not prompt:
            send_to_client(event, connection_id, {
                "type": "response",
                "response": "Hello! I'm the Device Management Agent. I can help you monitor device fleet health, diagnose issues, predict failures, and manage device-patient assignments. What would you like to know?",
            })
            return {"statusCode": 200}
