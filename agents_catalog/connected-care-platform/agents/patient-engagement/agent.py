"""Patient Engagement Agent — Entry Point with AgentCore Memory"""
...
def invoke(payload: dict) -> dict:
    ...
    if not prompt:
        return {"response": "Hello! I'm the Patient Engagement Agent.", "session_id": session_id}
    ...
    try:
        ...
        # Any failure (Decimal types, corrupted session, etc.) — retry without memory
        import logging
        logging.getLogger(__name__).error(f"PE_AGENT_FAIL: {str(e)[:200]}, retrying stateless")
        agent = Agent(model=AGENT_MODEL, system_prompt=SYSTEM_PROMPT, tools=ALL_TOOLS)
        try:
            result = agent(prompt)
        except Exception as e2:
            # If stateless also fails, return the error
            return {"response": f"Agent error: {str(e2)[:200]}", "session_id": session_id, "memory": {"used": False, "short_term": 0, "long_term": []}}
...
if __name__ == "__main__":
    if os.environ.get("LOCAL_MODE", "false").lower() == "true":
        print("Patient Engagement Agent — Local Testing Mode\nType 'quit' to exit.\n")
        while True:
