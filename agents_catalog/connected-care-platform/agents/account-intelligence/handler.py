"""Account Intelligence Agent — WebSocket Lambda Handler"""
...
connections_table = dynamodb.Table(os.environ.get("CONNECTIONS_TABLE", "connected-care-ai-connections"))
...
def handler(event, context):
    if event.get("isOrchestrator"):
        ...
        if not prompt:
            return {"statusCode": 200, "body": json.dumps({"response": "Account Intelligence Agent ready."})}
        result = agent(prompt)
    ...
    try:
        ...
        prompt = body.get("prompt", "")
        if not prompt:
            send_to_client(event, connection_id, {"type": "response", "response": "Hello! I'm the Account Intelligence Agent. I can help you assess account health, predict contract renewal risks, and identify revenue opportunities across your hospital customers."})
            return {"statusCode": 200}
